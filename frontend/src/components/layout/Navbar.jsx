...
import { Sun, Moon, Menu, X, ShieldCheck, Cpu } from 'lucide-react';
...
const Navbar = () => {
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Link to="/" className="flex items-center gap-4 group">
                        ...
                        <div className="flex flex-col">
                            <span className="text-sm font-black tracking-[0.25em] text-primary uppercase group-hover:text-accent transition-colors duration-500">MHCV</span>
                            <span className="text-[7px] font-black tracking-[0.5em] text-primary/30 uppercase">Software Developer</span>
                        </div>
                    </Link>
                        ...
                        <div className="flex items-center gap-10">
                            {navLinks.map((link) => (
                                <NavLink
                                    ...
                                    to={link.path}
                                    className={({ isActive }) =>
                                        `relative group py-2 flex items-center gap-2 text-[10px] font-black font-heading uppercase tracking-[0.3em] transition-all duration-700
                                        ${isActive ? 'text-accent' : 'text-primary/40 hover:text-primary'}`
                                    }
                                >
                                    <span className="relative z-10">{link.name}</span>

                                    {/* Advanced Interaction Indicator */}
                                    <motion.div
            ...
            {/* Mobile Registry Interface - Enhanced Glassmorphism */}
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                            ...
                            {navLinks.map((link, idx) => (
                                ...
                                >
                                    <NavLink
                                        ...
                                        className={({ isActive }) =>
                                            `flex items-center justify-between px-8 py-6 rounded-[2rem] transition-all duration-700
